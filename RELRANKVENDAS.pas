...
begin
  ...
  // Define a nova consulta SQL completa, corrigindo a conversão de campos com vírgula
  QRYRKVEND.SQL.Text :=
    'SELECT ' +
    '    P.Produto, ' +
    '    P.CodInterno, ' +
    '    SUM(CAST(REPLACE(ISNULL(I.Total, ''0''), '','', ''.'') AS DECIMAL(18, 2)) - ' +
    '        CAST(REPLACE(ISNULL(I.DescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2)) + ' +
    '        CAST(REPLACE(ISNULL(I.AcrescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2))) AS ValorTotal, ' +
    ...
    '    P.naosaitabela, ' +
    '    P.MOEDA, ' +
    '    (SELECT SUM(CAST(REPLACE(ISNULL(T.Total, ''0''), '','', ''.'') AS DECIMAL(18, 2)) - ' +
    '                    CAST(REPLACE(ISNULL(T.DescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2)) + ' +
    '                    CAST(REPLACE(ISNULL(T.AcrescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2))) ' +
    ...
    'WHERE ' +
    '    CAST(REPLACE(ISNULL(I.Total, ''0''), '','', ''.'') AS DECIMAL(18, 2)) > 0 ' +
    '    AND V.Cancelada <> 1 ' +
    '    AND V.Venda = 1 ' +
    '    AND V.Status = ''V'' ' +
...
begin
  ...
  try
    ...
    SQLText.Add('    SUM(CAST(REPLACE(ISNULL(I.Total, ''0''), '','', ''.'') AS DECIMAL(18, 2)) - ' +
                '        CAST(REPLACE(ISNULL(I.DescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2)) + ' +
                '        CAST(REPLACE(ISNULL(I.AcrescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2))) AS ValorTotal, ');
    ...
    SQLText.Add('    P.MOEDA, ');
    SQLText.Add('    (SELECT SUM(CAST(REPLACE(ISNULL(T.Total, ''0''), '','', ''.'') AS DECIMAL(18, 2)) - ' +
                '                    CAST(REPLACE(ISNULL(T.DescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2)) + ' +
                '                    CAST(REPLACE(ISNULL(T.AcrescRateio, ''0''), '','', ''.'') AS DECIMAL(18, 2)))');
    ...
    if not (Trim(EditLocaliza.Text) = '') then
      SQLText.Add('     AND P2.MOEDA LIKE :Localizacao ');
    ...
    if not (Trim(EditLocaliza.Text) = '') then
      SQLText.Add('    AND P.MOEDA LIKE :Localizacao ');
    ...
    if not (Trim(EditLocaliza.Text) = '') then
      QRYRKVEND.ParamByName('Localizacao').AsString := '%' + Trim(EditLocaliza.Text) + '%';
